...
아래 여섯 경로, 요청·응답 필드, HTTP status와 다섯 공개 오류의 의미는 이번 구현의 확정 계약이다.
...
- 기존 연결이 있는데 callback에서 다른 워크스페이스가 확인되면 새로 발급된 자격을 저장하지 않고 폐기하며 기존 연결을 유지한다. callback은 `NOTION_REAUTH_REQUIRED`로 복귀시키고, 클라이언트는 같은 워크스페이스를 다시 승인하거나 현재 연결 해제 뒤 다른 워크스페이스 연결을 시작한다.
...
- callback redirect query는 `outcome=connected|cancelled|failed`와 `failed`일 때 다섯 Notion 공개 오류 중 하나인 `error`만 허용한다. `connected|cancelled`에는 `error`를 붙이지 않는다. Notion authorization `code`, access·refresh token, OAuth `state`, 원시 오류와 내부 예외는 복귀 URI에 포함하지 않는다.
...
- Markdown 응답의 첫 줄을 제목으로 간주해 제거하지 않는다. 정확한 `<br>` 문자열만 Markdown 강제 줄바꿈인 두 칸과 줄바꿈(`  \n`)으로 단순 치환하고, 그 밖의 enhanced Markdown과 링크는 응답 텍스트 그대로 유지한다.
- 이미지·영상·파일 등 미디어 본체와 녹취는 본문에서 제외하되 제공된 캡션·대체 텍스트는 일반 텍스트로 유지한다. 미디어 원본 URL, 다운로드 자격과 녹취 원문을 반환하지 않는다.
- Markdown 응답이 truncated 상태이거나 `unknown_block_ids`가 하나 이상이거나 본문에 `<unknown>`이 있으면 추가 회수를 시도하지 않고 `NOTION_CONTENT_INCOMPLETE`로 전체 실패한다. 이 실패에는 부분 `content`를 반환하지 않는다.
...
다섯 Notion 오류 코드, HTTP status와 사용자 복구 의미는 확정이다.
...
| `truncated`, `unknown_block_ids` 또는 `<unknown>`으로 완전한 본문을 확인할 수 없음 | `400` | `NOTION_CONTENT_INCOMPLETE` | 부분 본문을 사용하지 않고 선택 페이지를 보존해 재시도하거나 붙여넣기로 전환 |
...
- MVP의 안정 오류 코드는 `COMMON_001/002/003/999`, `AUTH_005`, `MATERIAL_001/002`, `NOTION_CONNECTION_REQUIRED`, `NOTION_REAUTH_REQUIRED`, `NOTION_PAGE_NOT_ACCESSIBLE`, `NOTION_CONTENT_INCOMPLETE`, `NOTION_TEMPORARILY_UNAVAILABLE`, `QUIZ_001/002`, `ATTEMPT_001`, `REVIEW_001`로 제한한다.
- Notion 오류는 위 다섯 가지 사용자 복구 의미로만 공개한다. 제공자 원시 오류명, HTTP 본문, 세부 block 타입, token 갱신 실패 원문과 내부 예외를 새 공개 코드나 `fields`로 전달하지 않는다.
...
- `error.message`는 사용자가 다음 행동을 이해할 수준으로 쓰고 내부 예외·LLM·Notion 상세를 노출하지 않는다. 정확한 사용자 문구는 이 계약의 안정 필드가 아니며 클라이언트는 다섯 Notion `error.code`와 [학습자료 흐름의 보존·복구 규칙](../ux/flow-content-import.md#취소와-실패)으로 분기한다.
...
- 다섯 Notion 오류의 정확한 사용자 문구와 화면 표현은 [학습자료 흐름의 열린 질문](../ux/flow-content-import.md#열린-질문)이 책임진다. 오류 코드, 보존 상태와 복구 행동은 확정 계약이다.
