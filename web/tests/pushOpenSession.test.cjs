...
function setup(options = {}) {
  ...
  const notification = { notificationId: uuid(2), quizSetId: uuid(3), materialId: uuid(4), actionType: 'FOCUS_QUIZ_IN_LIST', targetAvailable: true, createdAt: new Date().toISOString() }
  const {load} = fixture({
...
test('storage failure never ACKs and notification 404 recovers without a read intent',async()=>{
  const h=setup({storageFailure:true});try {h.session.receive(h.raw());await settle();assert.equal(h.sent.length,0)}finally{h.session.stop()}
  const j=setup({get:async()=>{throw {status:404}}});try{j.session.receive(j.raw());await settle();assert.equal(j.paths[0],'/notifications?unavailable=1');assert.equal(j.sent[0][1].outcome,'UNAVAILABLE');assert.equal(j.reads.size,0)}finally{j.session.stop()}
})
...
test('stale epoch and anonymous selection cannot access private results',async()=>{
 const h=setup();try{const old=h.raw();h.context.advanceAuthContext(99);h.session.receive(old);await settle();assert.equal(h.sent.length,0);assert.equal(h.paths.length,0);h.phase.setAuthPhase('anonymous');h.session.receive(h.raw());await settle();assert.equal(h.paths.length,0)}finally{h.session.stop()}
})
