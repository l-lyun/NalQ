...
- 참조 보존을 위해 `WITHDRAWN` tombstone을 유지하더라도 원 이메일 또는 원 이메일로 되돌릴 수 있는 값을 보관하지 않는다. 구체적인 저장 형태와 synthetic 값은 후속 계약·TRD에서 정한다.
...
- 탈퇴 서비스는 원 이메일의 식별·고유값 점유 해제와 `WITHDRAWN` 전환을 하나의 확정 결과로 처리해야 한다. 감사용 기록에는 이메일 원문이나 가역값을 남기지 않는다.
...
- 탈퇴 이메일을 제거·비식별화하면서 현재 `NOT NULL`·`UNIQUE` 제약을 만족할 구체적인 저장 형태와 synthetic 값
