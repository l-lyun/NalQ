...
이 문서는 [제품 기반의 최상위 탭 전환 모션](../../../docs/product.md#최상위-탭-전환-모션)과 홈·학습·프로필 화면 명세를 웹에서 구현하는 앱 내부 경계를 기록한다. 사용자 행동과 화면 구조는 상위 원장을 따르며 이 문서가 새 정책을 정하지 않는다.
...
- 공통 바텀 탭은 움직이지 않는 셸 평면에 두고 콘텐츠 panel만 `홈 → 학습 → 프로필` 순서에 따라 수평 전환한다. reduced motion에서는 위치 이동 없이 짧은 opacity 전환만 사용한다.
- 같은 목적지의 중복 선택은 무시한다. 전환 중 다른 탭 선택은 앞선 중간 목적지를 history에서 replace해 최신 목적지만 남긴다.
- `/learning/materials`, `/learning/materials/:materialId`, `/learning/quizzes`, `/learning/new`는 학습 하위 route로 등록하며 공통 탭을 숨긴다. 기존 학습 화면의 내부 history state를 route adapter가 갱신해 자료 상세와 새 문제 만들기 화면을 직접 진입시킨다.
