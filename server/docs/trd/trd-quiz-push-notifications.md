...
| `push/service/PushScheduler.java` | 발송·receipt·lease 복구·정리의 제한 실행. 자체 트랜잭션 없음 |
...
| `push/repository/PushDeliveryRepository.java` | 작업 저장·조회·조건부 결과 반영 |
| `push/repository/PushDeliveryClaimStore.java` | 복잡한 MySQL due/lease claim SQL 전용 저장소. 하나의 concrete 구현으로 시작 |
| `push/repository/PushDeviceOperationRepository.java` | 멱등 결과 조회·저장·만료 삭제 |
...
| `push/config/PushProperties.java`, `PushConfiguration.java` | 설정 검증, HTTP 클라이언트, 전용 제한 실행 자원 구성 |
...
- due 조회: `(state,nextAttemptAt,id)`, lease 복구: `(state,leaseUntil,id)`, receipt: `(state,receiptNextAt,id)`, 정리: createdAt/expireAt 및 `(status,inactiveAt,id)` 인덱스를 각 용도에 맞게 둔다. 실제 EXPLAIN과 동시성 테스트로 검증한다.
...
| 항목 | 시작값 |
| --- | --- |
| 발송 활성화 | false, enqueue와 전송 모두 차단; 정리·이미 접수된 receipt 확인은 별도 |
...
| HTTP/lease | 연결 3초, 전체 호출 deadline 10초 / lease 60초. 기본 HTTP 재시도는 끄고 worker가 횟수 관리 |
