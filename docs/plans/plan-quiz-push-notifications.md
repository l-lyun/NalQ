...
발송 제공자는 Expo Push Service, MVP 앱 구분은 일반 웹에서 등록 흐름 제외로 확정됐다. 공유 Contract 초안에 사용자 인증·기기 소유권, 해제 자격, 계정 전환 revision, 오류, 구버전 웹/앱 호환과 1시간 발송 기한을 구체화했다. 발송 기록 30일을 선택했고 재설치 복구 제안을 추가했다. 비활성 설치·멱등 기록 수명과 PRD의 개인정보·국외 이전 출시 요건은 별도 확인이 필요하다.
