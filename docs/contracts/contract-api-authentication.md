...
6자리 이메일 인증 코드를 전제로 한 2단계 가입, 닉네임 확인, 필수 동의, 로그인, 인증 상태 갱신, 현재 세션 로그아웃, 현재 사용자 확인과 회원 탈퇴의 클라이언트·서버 경계를 정의한다.
...
- 기존의 “`POST /api/v1/auth/sign-ups`에서 대기 사용자 생성” 계약은 서버에서 제거됐다. 웹 1단계도 이메일만 `POST /api/v1/auth/email-verifications`로 보내며 개발 환경의 인증 우회를 제공하지 않는다.
...
| 현재 사용자 닉네임 변경 | `PATCH /api/v1/users/me` | Access Token | `200 OK` |
| 현재 사용자 회원 탈퇴 | `DELETE /api/v1/users/me` | Access Token + 현재 비밀번호 | `200 OK` |
...
- DB 가입 확정 뒤 자동 로그인 세션 발급에 실패하면 계정과 약관 동의는 유지하고 `503 AUTH_011`을 반환한다. 클라이언트는 같은 가입 요청을 재전송하지 않고 비밀번호와 가입 계속 자격을 지운 뒤 가입 이메일만 채운 로그인 화면으로 이동한다.
...
- 활성 사용자는 자신의 표시 닉네임만 변경할 수 있다. 이메일과 다른 계정 정보는 이 요청으로 변경하지 않는다.
- 닉네임 형식, NFC 정규화와 영문 대소문자를 구분하지 않는 전역 중복 규칙은 가입과 동일하다.
- 현재 사용자가 이미 쓰는 닉네임을 다시 제출하는 요청은 성공한다. 다른 사용자가 사용 중이면 `409 AUTH_010`을 반환한다.
- 사용 가능 확인 결과는 닉네임을 예약하지 않는다. 최종 변경 요청에서 형식과 중복을 다시 검사한다.
- 성공 응답은 변경된 값을 포함한 [현재 사용자](#현재-사용자) 응답과 같다.
...
- 활성 Access Token의 내부 `userId`만 탈퇴 대상으로 사용하며 요청에서 별도 사용자 ID나 이메일을 받지 않는다.
- `withdrawalRequestId`는 클라이언트가 안전한 난수원으로 생성한 UUID다. 같은 사용자와 같은 요청 ID의 재전송은 최초 확정 결과를 반환하며 상태 전이·세션 폐기·후속 삭제 작업을 중복 생성하지 않는다.
- `currentPassword`는 필수이며 현재 계정의 Argon2id 해시와 일치해야 한다. 비밀번호 재설정 코드나 이메일 코드는 이 endpoint에서 받지 않는다.
- `confirmation`은 Unicode 문자열 `회원탈퇴`와 정확히 일치해야 한다. trim, 공백 허용 또는 유사 문자열 보정을 적용하지 않는다.
- 비밀번호와 확인 문구 검증이 모두 끝나기 전에는 사용자·세션·후속 삭제 상태를 변경하지 않는다.
...
```json
{
  ...
  "data": {
    "withdrawalRequestId": "018f5f95-61c7-7d7b-9f8c-6cb4a9b16731",
    "status": "WITHDRAWN",
    "withdrawnAt": "2026-09-03T06:00:00Z",
    "dataDisposalDeadline": "2026-10-03T06:00:00Z"
  },
  "error": null
}
```

- DB 확정 단위는 `WITHDRAWN` 전환, `withdrawnAt`, 요청 ID, 최대 30일 처리 기한, 직접 계정 식별정보·자격정보 제거와 이메일·닉네임 고유값 점유 해제다.
- DB 확정 뒤 모든 Redis Refresh session/family를 사용자 단위로 폐기한다. DB와 Redis의 분산 트랜잭션을 요구하지 않으며, 모든 보호 API가 DB의 활성 사용자 상태를 확인해 `WITHDRAWN` 사용자의 접근과 갱신을 즉시 거절하는 것을 보안 원장으로 삼는다.
- 브라우저 호출은 성공 응답에서 발급 때와 같은 속성으로 Refresh Cookie를 만료시킨다. 네이티브와 웹은 성공 즉시 로컬 Access/Refresh Token과 개인 cache를 제거한다.
- 동일한 `withdrawalRequestId`로 결과를 재확인할 때는 아직 서명·만료 검증을 통과하는 기존 Access Token에서 subject만 확인하고 같은 성공 응답을 반환할 수 있다. 이 예외는 다른 보호 데이터 접근이나 새 토큰 발급을 허용하지 않는다.
- 다른 요청 ID로 이미 탈퇴한 계정에 접근하거나 탈퇴 뒤 로그인·갱신을 시도하면 일반 비활성 인증과 같은 `AUTH_005`를 반환해 계정 상태를 추가로 공개하지 않는다.
- `dataDisposalDeadline`은 데이터를 30일간 보관한다는 의미가 아니라 삭제·비식별화를 완료해야 하는 최대 시각이다. 구체 처리 대상과 예외는 [사용자·인증 데이터 계약](contract-data-authentication.md#삭제와-보존)을 따른다.
...
| 닉네임 중복 또는 최종 가입·변경 전 선점 | `409` | `AUTH_010` | 닉네임 변경 후 중복 재확인 |
| 가입 확정 뒤 자동 로그인 세션 발급 실패 | `503` | `AUTH_011` | 가입 완료를 재시도하지 않고 잠시 후 로그인 |
...
| 탈퇴 현재 비밀번호 불일치 | `401` | `AUTH_012` | 현재 비밀번호 재입력 또는 탈퇴 흐름을 나가 비밀번호 재설정 |
| 탈퇴 확인 문구 불일치·요청 ID 형식 오류 | `400` | 기존 `COMMON_001` | 해당 필드 수정 |
| 탈퇴 DB 확정 실패 | `503` | `AUTH_013` | 같은 `withdrawalRequestId`로 결과 재확인 |
...
- 탈퇴 요청의 현재 비밀번호를 DB·Redis·애플리케이션 로그·프록시 로그·분석 사건·오류 추적 payload에 남기지 않는다. `withdrawalRequestId`, 내부 사용자 ID, 결과 코드와 처리 시각만 운영 추적에 사용할 수 있다.
...
- 서버가 `AUTH_011`을 반환하면 가입은 이미 확정된 상태다. 브라우저는 가입 완료를 반복하지 않고 비밀번호와 `signUpToken`을 폐기한 뒤 이메일만 보존해 로그인으로 안내한다.
- 최종 가입 결과를 알 수 없는 네트워크 오류나 안정적인 오류 코드가 없는 5xx 응답이면 브라우저는 Cookie 도착 여부를 확인하기 위해 refresh를 정확히 한 번 호출한다. 성공하면 복구된 세션을 사용하고, 실패하면 가입 완료를 반복하지 않고 로그인한다. 네이티브는 세션 body를 잃으면 로그인으로 복구한다.
- 갱신은 회전 때문에 일반적인 멱등 요청이 아니다. 소비자는 동시에 하나만 호출하고 실패 시 이전 자격을 무한 재시도하지 않는다.
- 탈퇴 응답을 받지 못한 경우 새 요청 ID를 만들거나 로그인·가입을 즉시 반복하지 않고 같은 `withdrawalRequestId`로 한 번 결과를 재확인한다. 같은 ID는 최초 성공 결과를 반환하고 최초 처리가 실패했다면 다시 한 번 확정을 시도할 수 있다.
