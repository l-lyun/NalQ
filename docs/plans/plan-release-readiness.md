...
- 제공자 응답은 provider-neutral 후보 모델로 변환한 뒤 기존 유형별 검증을 통과시킨다.
- 객관식 보기 3~5개와 단일 정답, 빈칸 marker·정답 연결, 단답형 허용 답안, 서술형 모범 답안·핵심 포인트·해설·source excerpt 등 현재 Contract의 유형별 필수값을 검증한다.
- 외부 문제 번호는 신뢰하지 않고 유효 후보의 원래 배열 순서에 `1..N`을 다시 부여한다.
- 요청한 일부 유형이나 최대 개수를 채우지 못해도 유효 문제가 하나 이상이면 `READY`, 유효 문제가 0개면 공개 failure 정책에 따라 `FAILED`로 확정한다.
