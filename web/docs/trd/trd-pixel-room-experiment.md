...
- 생성 원본이 실제 alpha 대신 밝은 체크 무늬를 포함하므로 원본 파일을 직접 변형하지 않고, `PixelSprite`가 실험용 SVG white-matte 표시 필터와 crop으로 프레임·모자 레이어를 합성한다. 색상 판별 뒤 만들어진 이진 전경 마스크를 원본 좌표 기준 2px 침식해 프레임 전환 때 남던 흰 외곽 혼합 픽셀을 숨긴다. 색상 임계값은 유지해 얼굴과 크림 모자의 넓은 밝은 면을 보존한다.
- white-matte와 마스크 침식은 밝은색 의상·아이템의 가장자리까지 지울 수 있어 정식 자산에는 사용할 수 없다. 제품 구현 전에는 검수된 투명 PNG로 교체해야 한다.
