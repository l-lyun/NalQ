...
- 본 퀴즈와 복습 결과 adapter는 각 결과 응답의 `reviewAvailable`을 화면 모델에 그대로 전달한다. 결과 하단 `복습하기`는 이 값이 `true`이고 현재 결과 재조회가 진행 중이지 않을 때만 현재 본 퀴즈 attempt ID 또는 복습 결과의 원본 attempt ID로 기존 review session 생성 API를 호출하며, 과거 `MAIN`을 나타내는 `false` 결과에서는 화면의 미해결 문항 수가 남아 있어도 노출하지 않는다. 성공하면 결과 route가 계속 활성 상태일 때만 반환된 session route로 이동하고, 사용자가 요청 중 `홈으로` 등으로 이탈했다면 늦은 응답은 cache invalidation까지만 반영하고 현재 위치를 바꾸지 않는다. 진행 중에는 중복 요청을 막으며, 실패하면 현재 결과를 유지한다. `홈으로`는 `/`로 이동한다.
...
- 실제 생성 요청 전에는 OpenAI로 전송하는 정보, 전송하지 않는 계정·풀이 정보와 입력·출력의 보관 기간을 `ContentDialog`에서 확인한다. 보관 문구는 통상 최대 30일인 기본과 법적 의무나 서비스·제3자 보호를 위한 장기 보관 예외를 함께 표시하고, 한 줄을 강제하지 않아 좁은 화면과 글자 확대에서 자연스럽게 줄바꿈한다. 현재 본문 SHA-256 지문이 기기 로컬 확인 지문과 같으면 다시 묻지 않고, 본문이 바뀌면 다시 확인하며 제목만 바뀐 경우는 기존 확인을 유지한다. 취소는 조건을 유지하고 요청하지 않으며 확인 행동 중 중복 요청을 막는다.
...
- 결과 화면에서 복습 세션 생성 중 `홈으로` 이탈하면 생성 응답이 늦게 도착해도 복습 route로 다시 이동하지 않는다.
- 본 퀴즈 또는 복습 결과의 `reviewAvailable=false`이면 미해결 문항 수가 남아 있어도 `복습하기`를 노출하지 않는다.
