...
최초 자동 판정과 최신 override만 보존하고 전체 수정 이력을 만들지 않는 선택은 사용자 확정 사항이다. 활성 복습 세션 snapshot을 유지하는 선택만 이번 초안의 권고안이며, 구현을 시작하기 전에 사용자와 최종 확인한다. 변경이 필요하면 제품 정책과 공유 계약을 먼저 수정한다.
