...
- 학습 메인의 `전체 문제 다시 풀기`는 `/quiz-sets/:quizSetId`에 `restartMain` route state를 전달한다. 이 intent에서는 미완료 자기평가 자동 재개 Query를 건너뛰고 `READY`부터 새 `MAIN` 회차를 시작하며, 일반 진입과 `자기평가 이어하기`는 기존처럼 pending 회차를 우선한다.
