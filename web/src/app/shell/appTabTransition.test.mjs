...
test('학습에서 홈으로 돌아와도 동일한 방향 class를 적용한다', () => {
  const transition = { from: 'learning', to: 'home' }
  assert.match(getTabPanelClassNames('home', 'home', transition), /app-tab-panel--from-right/)
  assert.match(getTabPanelClassNames('learning', 'home', transition), /app-tab-panel--to-left/)
})
