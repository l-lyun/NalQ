...
- `refreshExpiresAt`을 브라우저 세션 응답 body에 유지할지
- 기존 Cookie가 있는 상태에서 다른 계정 로그인에 성공할 때 이전 session/family를 즉시 폐기할지
- 다중 탭 경합과 refresh 성공 응답 유실을 strict 재사용 탐지로 fail-closed 처리할지, 제한된 grace 정책을 별도 설계할지
- Origin/CSRF 검증 실패에 사용할 안정적인 오류 코드
